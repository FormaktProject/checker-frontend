...
const CheckerSearchBox = ({ 
  ...
  const handleCountrySelect = (country: string) => {
    ...
    setFilteredcitys(getCitiesForCountry(country))
    setSelectedCity('');
                  ...
                  {filteredCountries.length > 0 ? (
                    filteredCountries.map((country, index) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm transition-all duration-150 ${highlightedIndex === index
                          ? 'bg-blue-50 text-blue-700'
                          : 'text-slate-600 hover:bg-slate-50'
                          }`}
                        onMouseEnter={() => setHighlightedIndex(index)}
                      >
                        <MapPin className={`h-4 w-4 transition-colors ${highlightedIndex === index
                          ? 'text-blue-500'
                          : 'text-slate-400'
                          }`} />
          ...
          <div className="relative" ref={dropdownRefcity}>
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                ...
                <ChevronDown
                  className={`h-4 w-4 text-slate-400 transition-transform duration-200 ${isOpencity ? 'rotate-180' : ''
                    }`}
                />
              </div>
            </div>
                  ...
                  {filteredcitys.length > 0 ? (
                    filteredcitys.map((city, index) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm transition-all duration-150 ${highlightedIndexcity === index
                          ? 'bg-blue-50 text-blue-700'
                          : 'text-slate-600 hover:bg-slate-50'
                          }`}
                        onMouseEnter={() => setHighlightedIndexcity(index)}
                      >
                        <MapPin className={`h-4 w-4 transition-colors ${highlightedIndexcity === index
                          ? 'text-blue-500'
                          : 'text-slate-400'
                          }`} />
